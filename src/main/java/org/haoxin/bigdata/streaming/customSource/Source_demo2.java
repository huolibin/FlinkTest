 ...
 */
public class Source_demo2 {
    public static void main(String[] args) throws Exception {
        //获取环境
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        //获取数据,并设置并行度，（默认的并行度是cup核数）
        //DataStreamSource<Long> text = env.addSource(new MyParallelSourceFunction()).setParallelism(2);
        DataStreamSource<Long> text = env.addSource(new MyParallelSourceFunction());
        //整理数据
        ...
        String jobName = Source_demo2.class.getSimpleName();
        env.execute(jobName);
